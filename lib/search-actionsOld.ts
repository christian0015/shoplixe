...
import { getSmartCityMatch } from '@/lib/city-helper';
...
/** Recherche texte sur les produits (Atlas Search + Fuzzy), filtrable par catégorie/ville. */
export async function searchProducts(
  ...
  if (query && query.trim()) {
    ...
    if (shopIds) pipeline.push({ $match: { shop: { $in: shopIds.map((id) => new mongoose.Types.ObjectId(id)) } } });
    pipeline.push({ $limit: 40 });

    const products = await Product.aggregate(pipeline);
    await Product.populate(products, { path: 'shop', select: 'slug name city' });

    return products.filter((p: any) => p?.shop?.slug).map((p) => toProductResult(p as unknown as Record<string, unknown>));
  }

  const filter: Record<string, unknown> = { available: true };
  if (shopIds) filter.shop = { $in: shopIds };

  const products = await Product.find(filter).limit(40).populate('shop', 'slug name city').sort({ createdAt: -1 }).lean();
  return products.filter((p: any) => p?.shop?.slug).map((p) => toProductResult(p as unknown as Record<string, unknown>));
}
...
/** Recherche géospatiale des boutiques */
...
/** Recherche géospatiale des produits avec Atlas Search */
export async function searchNearbyProducts(
  ...
  if (query && query.trim()) {
    const pipeline: any[] = [
      ...
      { $match: { available: true, shop: { $in: shopObjIds } } },
      { $limit: 40 },
    ];
  ...
  const products = await Product.find({ available: true, shop: { $in: shopObjIds } }).limit(40).populate('shop', 'slug name city').sort({ createdAt: -1 }).lean();
  return products.filter((p: any) => p?.shop?.slug).map((p) => toProductResult(p as unknown as Record<string, unknown>));
}